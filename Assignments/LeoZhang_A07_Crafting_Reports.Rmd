...
```{r chunk_1, message = FALSE, warning = FALSE,             results = 'hide'}
#Import libraries
library(tidyverse)
library(here)
library(knitr)
...
'EPAair_O3_PM25_NC1819_Processed.csv'), 
stringsAsFactors = TRUE) %>% mutate(Date = ymd(Date))
```
...
summary_PM25 <- epa_data %>% drop_na(PM2.5) %>% group_by(COUNTY) %>% summarize(mean_PM2.5 = mean(PM2.5)) %>% arrange(mean_PM2.5)
#Compute mean ozone concentration for each county, sorted by value
summary_Ozone <- epa_data %>% drop_na(Ozone) %>% group_by(COUNTY) %>% summarize(mean_Ozone = mean(Ozone)) %>% arrange(mean_Ozone)
#Use the `kable` function to produce the 2 tables in markdown format
kable(summary_PM25, digits = 2, row.names = NA, col.names = c('County', '$\\mu g/m^3$'), align = 'l', caption = 'Mean Particulates (2.5mm)')
kable(summary_Ozone, digits = 2, row.names = NA, col.names = c('County', '$\\mu g/m^3$'), align = 'l', caption = 'Mean Ozone')
```
...
```{r chunk_3, echo = FALSE, fig.align = 'left',           fig.height = 3.9, fig.cap = 'Monthly Ozone (2018)'}
p1 <- epa_data %>% filter(Year == 2018) %>% ggplot(aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone)) + geom_boxplot(na.rm = TRUE, color = '#0096FF') + theme_gray() + labs(title = 'Monthly Ozone: 2018', x = '', y = expression(paste('Ozone(', mu, g/m^3, ')')))
p1
```


```{r chunk_4, echo = FALSE, fig.align = 'left',           fig.height = 3.9, fig.cap = 'Monthly Ozone (2019)'}
p2 <- epa_data %>% filter(Year == 2019) %>% ggplot(aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone)) + geom_boxplot(na.rm = TRUE, color = '#0096FF') + theme_gray() + labs(title = 'Monthly Ozone: 2019', x = '', y = expression(paste('Ozone(', mu, g/m^3, ')')))
p2
```
...
<https://github.com/leozhangzzgx/872_spring26>
